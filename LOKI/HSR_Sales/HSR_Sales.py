...
def testIntent():
    ...
    testLoki(inputLIST, ['arrival_time-2'])
    print("")
    ...
    testLoki(inputLIST, ['From-2'])
    print("")

    # THSR_Ticket_Who
    print("[TEST] THSR_Ticket_Who")
    inputLIST = ['一張普通票','優待票兩張','買三張全票','兩張老人的票','給我六張敬老票','兩張全票三張半票','可以給我六張老人票嗎','學生票和兒童票各兩張','普通票一張兒童票一張','全票學生票敬老票各一張','請幫我準備六張敬老票。','買兩張全票和三張半票。','麻煩您給我六張老年票。','學生票兒童票敬老票都來一張。','我想要各兩張學生票與兒童票。','我需要一張普通票和一張學生票','一張大人的票和一張小朋友的票。','我需要一張普通票加一張學生票。','請給我一張普通票和一張學生票。','我想購買一張普通票和一張學生票。','請幫我辦理兩張全票和三張半價票。','能不能把普通票跟學生票各給我一張？','能不能給我一張普通票與一張學生票？']
    testLoki(inputLIST, ['THSR_Ticket_Who'])
    print("")

    # Destination-2
    print("[TEST] Destination-2")
    inputLIST = ['要去台中','單程到台南','想要到臺中去','要買到台北的票','到臺南的單程車票','前往臺南的單程票','一張單程去臺南的票','打算買到臺北的票。']
    testLoki(inputLIST, ['Destination-2'])
    print("")
